-- Drop existing projects table and recreate with all columns
DROP TABLE IF EXISTS project_analytics CASCADE;
DROP TABLE IF EXISTS sustainability_recommendations CASCADE;
...
CREATE INDEX idx_projects_sustainability_score ON projects(sustainability_score);
...
CREATE INDEX idx_analytics_project_id ON project_analytics(project_id);
CREATE INDEX idx_analytics_event_type ON project_analytics(event_type);
